...
public class SimpleInventory : MonoBehaviour
{
    ...
    public List<ItemConfig> allItemConfigs = new List<ItemConfig>()
    {
        ...
        new ItemConfig() { itemID = "bread", itemName = "面包", defaultCount = 1 },
        new ItemConfig() { itemID = "apple_wine", itemName = "果酒", defaultCount = 1 }
    };
    
    private void Start()
    {
        // 手动添加初始物品（使用通用方法，会自动从配置获取图标）
        AddItem("wheat_seed", 4);  // 5个小麦种子
        // 同时加载编辑器配置的初始物品（可选）
    ...
    internal void AddItem(object itemID, int harvestCount)
    {
