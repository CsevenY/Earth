...
public class FarmPlot : MonoBehaviour
{
    // 外部绑定
    [Header("耕地配置")]
    // 核心修改：ItemDataSO → ItemCraftDataSO
    public ItemCraftDataSO[] availableSeeds; // 改为数组，这个耕地可用的种子类型
    ...
    // 当前检测到的可用种子
    // 核心修改：ItemDataSO → ItemCraftDataSO
    private ItemCraftDataSO detectedSeed = null;
    ...
    public void UpdatePromptWindow(bool isPlayerNear)
    {
        ...
        // 新增：检查是否有未成熟作物，显示浇水提示和进度
        Crop nearbyGrowingCrop = CheckForNearbyGrowingCrop();
        if (nearbyGrowingCrop != null && isPlayerNear)
        {
            plantPromptWindow.SetActive(true);
            // 计算生长进度百分比
            float progressPercent = (nearbyGrowingCrop.growthProgress / nearbyGrowingCrop.GrowthTime) * 100f;
            // 显示浇水提示和进度
            string waterStatus = nearbyGrowingCrop.isWatered ? "[已浇水]" : "";
            promptText.text = $"按Q键浇水 {waterStatus}\n生长进度：{progressPercent:F1}%";
    ...
    // 新增：检查附近是否有未成熟的生长中作物
    private Crop CheckForNearbyGrowingCrop()
    {
        ...
        // 1. 获取场景中所有未成熟的作物（注意：改为查找全局作物，而非单当前耕地）
        Crop[] allGrowingCrops = FindObjectsOfType<Crop>();
        List<Crop> inRangeCrops = new List<Crop>();

        // 2. 筛选出 未成熟 且 在检测范围内 的作物
        foreach (Crop crop in allGrowingCrops)
        {
            if (crop.CurrentState != Crop.CropState.Mature)
            {
                ...
                {
                    inRangeCrops.Add(crop);
                }
            }
        }

        // 3. 从范围内的作物中，找到离玩家最近的那个
        foreach (Crop crop in inRangeCrops)
        {
            float distance = Vector3.Distance(player.position, crop.transform.position);
    ...
    private void TryWaterCrop()
    {
        ...
        if (isProcessing)
        {
            Debug.Log($"【调试日志】{gameObject.name}：正在处理中，跳过浇水");
            return;
        }
        ...
        try
        {
            // 找到附近可浇水的作物
            Crop nearbyGrowingCrop = CheckForNearbyGrowingCrop();
            if (nearbyGrowingCrop != null)
            {
                bool waterSuccess = nearbyGrowingCrop.WaterCrop();
                if (waterSuccess)
                {
                    Debug.Log($"【调试日志】{gameObject.name}：成功给{nearbyGrowingCrop.CropName}浇水");
                    // 立即更新提示
                    UpdatePromptWindow(true);
                }
                else
                {
                    Debug.LogWarning($"【调试日志】{gameObject.name}：给{nearbyGrowingCrop.CropName}浇水失败");
                    if (promptText != null)
                    {
                        promptText.text = "该作物无需浇水！";
                        promptText.color = Color.red;
                    }
                }
            }
            ...
            {
                Debug.LogWarning($"【调试日志】{gameObject.name}：没有可浇水的作物");
                if (promptText != null)
                {
                    promptText.text = "没有可浇水的作物！";
                    promptText.color = Color.red;
                }
            }
        }
    ...
    // 获取当前可用且被选中的种子
    // 核心修改：返回值从ItemDataSO → ItemCraftDataSO
    private ItemCraftDataSO GetAvailableSelectedSeed()
    {
        ...
        {
            foreach (ItemCraftDataSO seed in availableSeeds)
            {
    ...
    private Crop CheckForNearbyMatureCrop()
    {
        ...
        Crop[] allMatureCrops = FindObjectsOfType<Crop>();
        List<Crop> inRangeCrops = new List<Crop>();

        foreach (Crop crop in allMatureCrops)
        {
            if (crop.CurrentState == Crop.CropState.Mature)
            {
                float distance = Vector3.Distance(player.position, crop.transform.position);
